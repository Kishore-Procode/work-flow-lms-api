-- Create Demo Users for Student-ACT LMS (Fixed Roles)
-- This script creates demo users with the correct enum roles
...
BEGIN
    ...
    -- Insert or update demo users with correct roles
    ...
    -- Principal User (using admin role since principal doesn't exist in enum)
    INSERT INTO users (
        id, name, email, password_hash, role, status, college_id, 
        email_verified, created_at, updated_at, phone
    ) VALUES (
    ...
    -- Staff User (using faculty role since staff doesn't exist in enum)
    INSERT INTO users (
        id, name, email, password_hash, role, status, college_id, department_id,
